...
def process_video():
    ...
    while True:
        ...
        (h, w) = frame.shape[:2]
        blob = cv2.dnn.blobFromImage(cv2.resize(frame, (450, 450)), 0.007843, (450, 450), mean) #576, 702
        net.setInput(blob)
        detections = net.forward()

        # Listy przechowujące bounding boxy, poziomy pewności i klasy detekcji
        boxes = []
        confidences = []
        class_ids = []
        ...
        # Stosowanie Non-Maximum Suppression (NMS)
        ...
        # Rysowanie ramek po zastosowaniu NMS
        if len(indices) > 0:
            for i in indices.flatten():
                ...
                # Aktualizacja liczby obiektów dla każdej klasy
        ...
        # Wyświetlanie liczby wykrytych obiektów
        y_position = 75
